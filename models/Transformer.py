...
class Transformer(nn.Module):
	...
	def __init__(self, args):
		...
		self.att_size = args.features_size
		self.embed_size = 2048#args.features_size#args.embed_size
		self.use_bn = args.use_bn
		self.drop_prob_lm = args.drop_prob_lm
		self.tgt_vocab_size = args.vocab_len
		print("target vocab size:", self.tgt_vocab_size)
		self.num_layers = args.num_layers
		self.ff_size = args.ff_size
		
		self.att_embed = nn.Sequential(*(((nn.BatchNorm1d(self.att_size),) if self.use_bn else ()) \
			+(nn.Linear(self.att_size, self.embed_size), nn.ReLU(),nn.Dropout(self.drop_prob_lm))  \
	...
	def evaluate(self, att_feats, max_len, start_symbol=1, att_masks=None):
		# print("att_feats1:", att_feats.size())
		att_feats = self.att_embed(att_feats)
		# print("att_feats2:", att_feats.size())
