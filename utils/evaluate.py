...
def language_eval(args, input_data, savedir, split, epoch, iteration):
  ...
  annFile = os.path.join(args.caption_path,'{}_list.json'.format(split))
  if args.use_coco:
  ...
  if (epoch+1) % args.save_step == 0:
    targetFile = osp.join(savedir, 'result_%s-%d-%d.json' % (split, epoch, iteration))
    os.system("cp {} {}".format(resFile, targetFile))
